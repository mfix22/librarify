...
for (var c in settings.config) {
  if (settings.config[c] == true || settings.config[c].required == true) readme += '* `' + c + '` _(required)_ - ' + (settings.config[c].desc || '') + '\n'
...
for (var fname in settings.fns) {
  readme += '#### ' + fname + '(options[, callback])' + ' `' + (settings.fns[fname].type || 'GET').toUpperCase() +'`\n'
